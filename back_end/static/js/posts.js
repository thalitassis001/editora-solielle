...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderLaunches() {
    ...
    if (!Array.isArray(LAUNCHES_DATA) || LAUNCHES_DATA.length === 0) {
      noLaunches.style.display = 'block';
    } else {
      noLaunches.style.display = 'none';
      launchGrid.innerHTML = LAUNCHES_DATA.map(b => `
  ...
  function renderRecommendations() {
    if (!recomList) return;
    recomList.innerHTML = RECOMMENDATIONS_DATA.map((r, i) => `
  ...
  function renderArticles() {
    ...
    if (!Array.isArray(ARTICLES_DATA) || ARTICLES_DATA.length === 0) {
  ...
  async function loadData() {
      try {
        const [launchesRes, recommendationsRes, articlesRes] = await Promise.all([
          fetch("/api/launches"),
          fetch("/api/recommendations"),  
          fetch("/api/articles")
        ]);

        LAUNCHES_DATA = await launchesRes.json();
        RECOMMENDATIONS_DATA = await recommendationsRes.json();
        ARTICLES_DATA = await articlesRes.json();

        renderLaunches(LAUNCHES_DATA);
        renderRecommendations(RECOMMENDATIONS_DATA);
        renderArticles(ARTICLES_DATA);
